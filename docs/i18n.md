...
| **Volunteer spotlight** (name, role, blurb) | `src/content/en/spotlights.json` (Decap CMS) | `src/content/es/spotlights.json` (Decap CMS) |
...
### Volunteer spotlight (Decap CMS)

- Edit spotlights at **/admin** → **Volunteer Spotlight (English)** or **Volunteer Spotlight (Spanish)**. The build reads `spotlights.json` from each content folder.
...
3. Create `src/content/<locale>/` and add one `.md` file per page (same names as in `en/`) and `spotlights.json` if the site uses spotlights (or create it via Decap CMS).
...
- The build script (`bb/build.clj`) loops over `locales` and, for each locale, reads i18n from `src/i18n/<locale>.yaml`, content from `src/content/<locale>/*.md`, and spotlights from `src/content/<locale>/spotlights.json`.
